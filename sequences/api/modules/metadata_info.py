...
class UserMetadataOnlyNameInfoSerializer(CustomSerializer):
	class Meta:
		model  = Metadata_Handler
		fields = [
			'user'
			...
			if(user_obj.is_active):
				...
				if(metadata_qs.count() > 0):
					...
					for index,i in enumerate(metadata_qs):
						temp.append(i.metadata)
					return_dict = itertools.chain(*temp)
					only_name = [i['Virus name'] for i in return_dict]
					return only_name
				return {'message': 'No Data Found'}
			raise serializers.ValidationError({'message': 'User Inactive'})
		raise serializers.ValidationError({'message': 'Invalid Credentials'})
...
class UserMetadataOnlyNameInfoAPI(generics.GenericAPIView):
	queryset                = Metadata_objects
	serializer_class        = UserMetadataOnlyNameInfoSerializer
	permission_classes      = []
