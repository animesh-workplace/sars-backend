rule top_mutation:
	...
	run:
		...
		for i in tqdm(all_states):
			writer_state = ExcelWriter(os.path.join(output.state_wise_mutation, 'mutations', f'{i}_mutations.xlsx'))
			temp = []
			state_metadata = metadata.iloc[metadata.index[metadata['division'].isin([i])].tolist()]
			state_metadata.to_csv(os.path.join(output.state_wise_mutation, 'metadata', f'{i}_metadata.tsv'), sep = '\t', index = False)
			state_sequences = [sequence[i]  for i in state_metadata['strain']]
			SeqIO.write(state_sequences, os.path.join(output.state_wise_mutation, 'metadata', f'{i}_sequence.fasta'), 'fasta')
