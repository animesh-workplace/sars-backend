rule split_state:
	...
	output:
		state_wise_path = directory(os.path.join("{base_path}/Analysis/{date}/reports", "state_wise"))
	log:
		os.path.join('{base_path}', 'Analysis', '{date}', 'log', 'split_state_error.log')
	run:
		try:
			...
			for i in all_states:
				...
				state_metadata.to_csv(os.path.join(output.state_wise_path, f"{i.replace(' ','_')}", f"{i.replace(' ','_')}_metadata.tsv"), sep = '\t', index = False)
				state_sequences = [sequence[i]  for i in state_metadata['strain']]
				SeqIO.write(state_sequences, os.path.join(output.state_wise_path, f"{i.replace(' ','_')}", f"{i.replace(' ','_')}_sequence.fasta"), 'fasta')
