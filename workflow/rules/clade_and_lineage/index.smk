rule get_lineage:
	...
	run:
		try:
			shell(
				"""
					pangolin {input} --outfile {output.lineage_report}
				"""
			)
		except:
...
rule combine_clade_pangolin:
	...
	input:
		...
		clade_label = rules.get_clade.output.clade_label,
		lineage_report = rules.get_lineage.output.lineage_report,
	output:
	...
	run:
		try:
			...
			nextclade_metadata = pandas.read_csv(input.clade_label, delimiter = '\t', encoding = 'utf-8', low_memory = False)
			...
			send_data_to_websocket('ERROR', 'combine_clade_pangolin', traceback.format_exc())
			raise
