    ...
    run:
        try:
            nextstrain_labels = [
                ...
                'segment', 'length', 'host', 'age', 'sex', 'originating_lab', 'submitting_lab',
                'authors', 'url', 'title', 'paper_url', 'date_submitted', 'purpose_of_sequencing'
            ]
            ...
            nextclade_pangolin = pandas.merge(
                nextclade_metadata[['strain', 'clade', 'totalInsertions', 'totalMissing', 'totalNonACGTNs', 'nonACGTNs',
                                    'substitutions', 'deletions', 'aaSubstitutions', 'aaDeletions']],
                pangolin_metadata[['strain', 'lineage', 'scorpio_call', 'note', 'pangoLEARN_version']],
                on='strain', how='inner'
            )

            region_type = pandas.read_csv('workflow/resources/indian_region.tsv',
                                          delimiter='\t', encoding='utf-8').set_index('State').T.to_dict()
            ...
            nextstrain_metadata = nextstrain_metadata.assign(
                collection_month=nextstrain_metadata['date'].dt.strftime('%b-%Y'),
                collection_week=nextstrain_metadata['date'].dt.strftime('W%V-%Y'),
                WHO_label="Others"
            )
            ...
            nextstrain_metadata.to_csv(
                output.nextstrain, sep='\t', index=False)
            ...
            if(not frontend_metadata['aaSubstitutions'].dropna().empty):
                all_variants = [mutations for (index, mutations) in frontend_metadata['aaSubstitutions']
                                .dropna().str.split(',').to_dict().items()]
            if(not frontend_metadata['aaDeletions'].dropna().empty):
                all_variants = all_variants + [mutations for (index, mutations) in frontend_metadata['aaDeletions']
                                               .dropna().str.split(',').to_dict().items()]
            ...
            week_keys = [i.strftime('W%V-%Y') for i in month_values]
            week_keys = pandas.DataFrame(week_keys)[0].unique().tolist()
            month_wise_mutation_percent = pandas.DataFrame(
                index=list(pandas.unique(frontend_nextstrain_metadata['WHO_label'])),
                columns=month_keys
            )
            week_wise_mutation_percent = pandas.DataFrame(
                index=list(pandas.unique(frontend_nextstrain_metadata['WHO_label'])),
                columns=week_keys
            )

            month_count = dict(collections.Counter(frontend_nextstrain_metadata['collection_month']))
            lineage_month_wise_count = pandas.crosstab(
            ...
            if(config['websocket']):
                send_data_to_websocket(
                    'SUCCESS_METADATA', 'combine_clade_lineage',
                    database_entry | storage.fetch("tool_version")
                )
        except:
