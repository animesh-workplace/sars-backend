...
logging.basicConfig(
    filename=f"{base_path}/Fixed_data/{date}/log/santize_data.log", format='%(asctime)s %(message)s', filemode='w'
)
...
def sanitize_data(rgsl):
    metadata_labels = [
        'Virus name', 'Type', 'Passage details/history', 'Collection date', 'Country', 'State', 'District', 'Location',
        'Additional location information', 'Host', 'Additional host information', 'Gender', 'Patient age', 'Patient status',
        'Specimen source', 'Outbreak', 'Last vaccinated', 'Treatment', 'Sequencing technology', 'Assembly method', 'Coverage',
        'Originating lab', 'Originating lab address', 'Submitting lab', 'Submitting lab address',
        'Sample ID given by the submitting lab', 'Authors'
    ]
    combined_metadata = pandas.DataFrame()
    indian_state_info = pandas.read_csv(
        "workflow/resources/indian_state_district.tsv", delimiter='\t', encoding="utf-8", low_memory=False
    )
    combined_sequences = []
                ...
                if(file_type == 'metadata'):
                    ...
                    if(os.path.isfile(sequence_url)):
                        try:
                            ...
                            print('Unable to read', sequence_url)
                    else:
                        print(f'{sequence_url} doesnot exist')
                    print(f'Metadata: {metadata_url} has length: {len(metadata)}')
                    print(f'Sequence: {sequence_url} has length: {len(sequence)}')
                    ...
                    if(metadata_ids - sequence_ids):
                        for ids in (metadata_ids - sequence_ids):
                            print(f'Metadata: {metadata_url} has additional ID: {ids}')
                            logger.info(f'Metadata: {metadata_url} has additional ID: {ids}')
                    if(sequence_ids - metadata_ids):
                        for ids in (metadata_ids - sequence_ids):
                            print(f'Sequence: {sequence_url} has additional ID: {ids}')
                            logger.info(f'Sequence: {sequence_url} has additional ID: {ids}')

                    fixed_gender = [
                        'Unknown' if(j != j or len(fs_gender.get(j)) > 1) else fs_gender.get(j)[0][1]
                        for j in metadata['Gender']
                    ]
                    fixed_state_name = [fs_state.get(j)[0][1] for j in metadata['State']]
                    fixed_collection_date = [
                        arrow.get(j, ['YYYY-MM-DD', 'DD-MM-YYYY', 'YYYY/MM/DD', 'DD/MM/YYYY', 'MM-DD-YYYY']).date()
                        for j in metadata['Collection date']
                    ]
                    ...
                    for (key, value) in sequence.items():
                        gc.disable()
                        if(key not in combined_sequences_label):
                    ...
                    SeqIO.write(
                        [seq for (key, seq) in sequence.items()], os.path.join(save_url, f"fixed_{sequence_name}"), 'fasta'
                    )
    print(
        f"Fixed metadata of {rgsl.split('_')[-1]}:" +
        f"Metadata - {len(combined_metadata.index)} Sequences - {len(combined_sequences)}"
    )
